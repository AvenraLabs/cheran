...
import { ORDERED_GOVERNMENT_STATUSES } from "../../scripts/seedStatuses.js";

const STATUS_LIFECYCLE_ORDER = {};
ORDERED_GOVERNMENT_STATUSES.forEach((name, idx) => {
  STATUS_LIFECYCLE_ORDER[name.trim().toUpperCase()] = (idx + 1) * 10;
});
// Common aliases
STATUS_LIFECYCLE_ORDER["WORK ORDER ISSUED"] = STATUS_LIFECYCLE_ORDER["ISSUED WORK ORDER"] || 220;
STATUS_LIFECYCLE_ORDER["SUPPLY OF MATERIALS"] = STATUS_LIFECYCLE_ORDER["INVOICED"] || 230;
STATUS_LIFECYCLE_ORDER["JV RECOMMENDED"] = STATUS_LIFECYCLE_ORDER["JOINT VERIFICATION COMPLETED"] || 520;

function getStatusStageWeight(statusName) {
  if (!statusName) return 5000;
  const upper = String(statusName).trim().toUpperCase();
  return STATUS_LIFECYCLE_ORDER[upper] ?? 5000;
}
